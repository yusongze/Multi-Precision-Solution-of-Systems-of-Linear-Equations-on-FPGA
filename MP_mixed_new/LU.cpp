 ...
 *
 ***********************************************************************/
void LU_factorization(	half A[N][N],
						float PA[N][N],
						float Pb[N]){
	int i,j,k;
	int Index = 0;
...
void LU_solver(	half A[N][N],
				float Pb[N],
				float x[N]){
	int i, j;
	half s;	//row partial sum.
	half y[N];
...
void LU_solver_refinement(	float PA[N][N],
							float Pb[N],
							half A[N][N],
							float x[N]){
	int i, j;
	...
	Rows:for(i=0;i<N;i++){
		...
		Columns:for(j=0;j<N;j++){
			...
			s_r += double(PA[i][j])*double(x[j]);
#ifdef fp_count
			fp_counter = fp_counter + 4;
#endif
		}
		r[i] = double(Pb[i])-s_r;		//compute residue
		y[i] = r[i]-s;
#ifdef fp_count
		fp_counter = fp_counter + 2;
#endif
...
void Iterative_refine(	float PA[N][N],
						float Pb[N],
						float x[N],
						half A[N][N]){
	int i;
	...
	//			write_to_file(norm_r, "norm_r.dat");
			...
			LU_solver_refinement(PA, Pb, A, x);
		}
}
...
void norm_2(double matrix[N], float *result){
	int i;
...
int maxIndex(half A[N][N], int columnIndex){
	int i, maxIndex;
	float maxValue;
